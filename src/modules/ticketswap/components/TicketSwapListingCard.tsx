...
import { Ticket, ArrowRight, MapPin, Sparkles } from 'lucide-react';
import { Listing } from '../../../shared/types';

interface TicketSwapListingCardProps {
  listing: Listing;
  isDarkMode: boolean;
...
export default function TicketSwapListingCard({
  listing,
  isDarkMode,
  isOwnListing,
  onSelect,
}: TicketSwapListingCardProps) {
  const savings = listing.originalPrice - listing.sellingPrice;

  return (
    <div
      id={`ticketswap-card-${listing.id}`}
    ...
    >
      {/* Dual indicator if the listing also has DayMates enabled! */}
      {listing.isDayMates && (
      ...
      {isOwnListing && !listing.isDayMates && (
      ...
      {savings > 0 && (
        <div
          ...
          aria-label={`₹${savings} off original rate`}
        >
          ₹{savings} Saved
        </div>
      )}
        ...
        <div className="mt-5 pt-3.5 border-t border-dashed border-gray-800">
          ...
          {listing.status === 'AVAILABLE' ? (
            <button
              ...
              aria-label={`Initiate escrow purchase swap process for movie ticket seat ${listing.seatNumber} of ${listing.movieName} at discount rate ₹${listing.sellingPrice}`}
