...
import { 
  Ticket, Compass, PlusSquare, MessageSquare, Wallet as WalletIcon, 
  Settings, Users, ShieldAlert, Sparkles, Filter, Trash2, ArrowRight, 
  Info, ShieldCheck, Heart, CircleDot, AlertTriangle, ChevronRight, HelpCircle,
  LogOut, Sun, Moon, User
} from 'lucide-react';

const LandingPage = React.lazy(() => import('./LandingPage'));
const ListingForm = React.lazy(() => import('./ListingForm'));
const WalletTab = React.lazy(() => import('../modules/profile/pages/WalletTab'));
const AdminPanel = React.lazy(() => import('../modules/profile/pages/AdminPanel'));
const ChatRoom = React.lazy(() => import('./ChatRoom'));
import LoginPage from '../shared/auth/LoginPage';
const ProfileTab = React.lazy(() => import('../modules/profile/pages/ProfileTab'));
const TicketSwapListingCard = React.lazy(() => import('../modules/ticketswap/components/TicketSwapListingCard'));
const DayMatesListingCard = React.lazy(() => import('../modules/daymates/components/DayMatesListingCard'));

import { SYSTEM_PERSONAS } from '../shared/constants';
import { Listing, Transaction, Wallet, Persona } from '../shared/types';
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'HOME' | 'BROWSE' | 'POST' | 'MESSAGES' | 'WALLET' | 'ADMIN' | 'PROFILE'>(() => {
  ...
  const navigate = (tab: 'HOME' | 'BROWSE' | 'POST' | 'MESSAGES' | 'WALLET' | 'ADMIN' | 'PROFILE', personaId = activePersona.id) => {
  ...
  const [transactionsPromise, setTransactionsPromise] = useState<Promise<Transaction[]>>(() => 
    fetch("/api/admin/stats").then(r => r.json().then(d => d.transactions as Transaction[]))
  );
  const [walletPromise, setWalletPromise] = useState<Promise<Wallet | null>>(() => 
    fetch(`/api/wallets/${activePersona.id}`).then(r => r.json() as Promise<Wallet>)
  );

  // Search/Filters (Browse page)
  const [searchQuery, setSearchQuery] = useState("");
  const [filterMode, setFilterMode] = useState<'ALL' | 'SAVINGS_HIGH'>('ALL');
  const [activeCategory, setActiveCategory] = useState<'TicketSwap' | 'DayMates'>('TicketSwap');
  ...
  const [checkoutMode, setCheckoutMode] = useState<'CONNECT' | 'SAFE' | null>(null);
  const [checkoutStep, setCheckoutStep] = useState<'DETAILS' | 'PAYING' | 'SUCCESS'>('DETAILS');
  ...
  const syncBackendData = async (tab = activeTab, personaId = activePersona.id) => {
    if (!personaId) return;
    try {
      if (tab === 'HOME' || tab === 'BROWSE') {
        setListingsPromise(fetch("/api/listings").then(r => r.json() as Promise<Listing[]>) );
      }
      if (tab === 'HOME' || tab === 'MESSAGES' || tab === 'ADMIN' || tab === 'PROFILE') {
        setTransactionsPromise(fetch("/api/admin/stats").then(r => r.json().then(d => d.transactions as Transaction[])) );
      }
      if (tab === 'WALLET' || tab === 'HOME' || tab === 'PROFILE') {
        setWalletPromise(fetch(`/api/wallets/${personaId}`).then(r => r.json() as Promise<Wallet>) );
      }
    } catch (err) {
  ...
  const handleCreateListing = async (formData: any) => {
    try {
      ...
      if (res.ok) {
        setGlobalSuccess("Movie listing posted successfully! Available to browse instantly.");
        navigate('BROWSE');
  ...
  const handleCheckoutSubmit = async (mode: 'CONNECT' | 'SAFE') => {
    ...
    const totalCost = mode === 'CONNECT' ? 5 : selectedListing.sellingPrice + platformFee;
  ...
  const handleWalletRefill = async () => {
    try {
      const refillPromise = fetch(`/api/wallets/${activePersona.id}/refill`, { method: "POST" })
        .then(r => r.json() as Promise<Wallet>);
      setWalletPromise(refillPromise);
      setGlobalSuccess("Demo Wallet funded with ₹500 successfully via simulated Razorpay UPI!");
  ...
  const filteredListings = listings.filter(l => {
    // Check if the listing matches the selected active category (dual listing means it can match both!)
    const isTS = l.isTicketSwap ?? (l.category === 'TicketSwap' || !l.category);
    const isDM = l.isDayMates ?? (l.category === 'DayMates');
    const matchesCategory = activeCategory === 'TicketSwap' ? isTS : isDM;
    if (!matchesCategory) return false;

    const matchesSearch = l.movieName.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          l.theatreName.toLowerCase().includes(searchQuery.toLowerCase());
    if (!matchesSearch) return false;
    
    if (filterMode === 'SAVINGS_HIGH') {
      if (activeCategory === 'TicketSwap') {
        return (l.originalPrice - l.sellingPrice) >= 100;
      } else {
        return l.sellingPrice === 0;
      }
    }
    return true;
  });
  ...
  const userTransactions = transactions.filter(t => t.buyerId === activePersona.id || t.sellerId === activePersona.id);

  const isSystemPersona = SYSTEM_PERSONAS.some(p => p.id === activePersona.id);

  return (
    <div className={`min-h-screen flex flex-col justify-between font-sans selection:bg-pink-600 selection:text-white transition-colors duration-200 ${
      ...
      {false && isSystemPersona && (
        <div className={`py-3 px-4 border-b sticky top-0 z-50 shadow-md transition-colors duration-250 ${
          isDarkMode ? 'bg-gray-950 border-gray-850' : 'bg-white border-slate-200 text-slate-900'
        }`}>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            ...
            {allPersonas.map((p) => (
              <button
                ...
                onClick={() => {
                  setActivePersona(p);
                  localStorage.setItem('ticketswap_active_persona', JSON.stringify(p));
              ...
              >
                <img src={p.avatar} className="w-4 h-4 rounded-full object-cover shrink-0" alt="" />
                <span className="capitalize">{p.name.split(' ')[0]} ({p.role})</span>
              </button>
            ))}
          </div>
      ...
      <header className={`backdrop-blur-md border-b sticky top-0 z-[40] transition-all duration-200 ${
        isDarkMode 
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
          ...
          <div className="flex items-center gap-2.5 cursor-pointer" onClick={() => navigate('HOME')}>
            ...
            <div>
              <span className={`text-lg font-bold font-display tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Ticket<span className="text-pink-500">Swap</span></span>
              <p className="text-[8px] font-mono tracking-widest text-zinc-500 uppercase leading-none mt-0.5">RESALE PLATFORM</p>
            </div>
          </div>
          ...
          <nav className="flex flex-wrap items-center justify-center gap-1 md:gap-2">
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold font-display transition duration-150 flex items-center gap-1 cursor-pointer ${
                activeTab === 'HOME' 
                  ? (isDarkMode ? 'bg-zinc-900 text-pink-500' : 'bg-pink-50 text-pink-600') 
                  : (isDarkMode ? 'text-gray-400 hover:text-white hover:bg-zinc-900/45' : 'text-slate-600 hover:text-slate-950 hover:bg-slate-100')
              }`}
              title="Overview"
            >
            ...
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold font-display transition duration-150 flex items-center gap-1 cursor-pointer ${
                activeTab === 'BROWSE' 
                  ? (isDarkMode ? 'bg-zinc-900 text-pink-500' : 'bg-pink-50 text-pink-600') 
                  : (isDarkMode ? 'text-gray-400 hover:text-white hover:bg-zinc-900/45' : 'text-slate-600 hover:text-slate-955 hover:bg-slate-100')
              }`}
        ...
        {(globalError || globalSuccess) && (
          <div className="max-w-4xl mx-auto px-4 mt-4">
            <div className={`p-4 rounded-xl text-xs font-semibold border flex items-center gap-2 ${
              globalError ? 'bg-rose-500/15 border-rose-500/20 text-rose-400' : 'bg-emerald-500/15 border-emerald-500/20 text-emerald-400'
            }`}>
        ...
        <React.Suspense fallback={
          ...
          {activeTab === 'BROWSE' && (
            <div className="browse-container max-w-6xl mx-auto px-2 sm:px-4 py-4 sm:py-8">
              ...
              <div className={`flex border-b mb-8 w-full transition ${isDarkMode ? 'border-gray-850' : 'border-slate-200'}`}>
                <button
                  onClick={() => setActiveCategory('TicketSwap')}
                  className={`px-5 py-3.5 text-sm font-bold font-display border-b-2 transition flex items-center gap-2 cursor-pointer ${
                    activeCategory === 'TicketSwap'
                      ? 'border-pink-500 text-pink-500 font-extrabold'
                      : (isDarkMode ? 'border-transparent text-gray-400 hover:text-white' : 'border-transparent text-slate-600 hover:text-slate-900')
                  }`}
                >
                  <Ticket className="w-4 h-4 rotate-12" />
                  TicketSwap (Resale Tickets)
                </button>
                <button
                  onClick={() => setActiveCategory('DayMates')}
                  className={`px-5 py-3.5 text-sm font-bold font-display border-b-2 transition flex items-center gap-2 cursor-pointer ${
                    activeCategory === 'DayMates'
                      ? 'border-pink-500 text-pink-500 font-extrabold'
                      : (isDarkMode ? 'border-transparent text-gray-400 hover:text-white' : 'border-transparent text-slate-600 hover:text-slate-900')
                  }`}
                >
                  <Users className="w-4 h-4" />
                  DayMates (Movie Companions)
                </button>
              </div>
              ...
              <div className="browse-gallery-header flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 mb-6 sm:mb-8">
                <div>
                  <h2 className={`text-2xl sm:text-3xl font-extrabold font-display ${isDarkMode ? 'text-white' : 'text-slate-900'}`} id="browse-gallery-title">
                    {activeCategory === 'TicketSwap' ? 'Available Resale Tickets' : 'Discover Movie Companions'}
                  </h2>
                  <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-slate-600'}`}>
                    {activeCategory === 'TicketSwap' 
                      ? 'Guaranteed cheaper than original price. Choose Safe Mode for 100% escrow protection.' 
                      : 'Find film buddies to share the screen, split booking costs, and watch releases together.'}
                  </p>
                </div>

                {/* Filters / Search Bar row */}
                <div className="flex flex-col sm:flex-row items-center gap-2.5" role="search" aria-label="Filter resales">
                  <input
                    type="text"
                  ...
                  <div className={`flex gap-1.5 p-1 rounded-xl border w-full sm:w-auto font-sans transition ${
                    isDarkMode ? 'bg-gray-900 border-gray-800/60' : 'bg-white border-slate-200 shadow-sm'
                  }`} role="group" aria-label="Discount rate filter toggle options">
                    <button
                      ...
                      className={`flex-1 sm:flex-initial px-3 py-2 sm:py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap cursor-pointer transition touch-target-btn ${
                        filterMode === 'ALL' 
                          ...
                          : (isDarkMode ? 'text-gray-400 hover:text-white' : 'text-slate-550 hover:text-slate-800')
                      }`}
                    >
                      All Seats
                    </button>
                    ...
                    >
                      {activeCategory === 'TicketSwap' ? 'High Savings (₹100+)' : 'Sponsored (Free)'}
                    </button>
                  </div>
                </div>
              </div>
              ...
              {filteredListings.length === 0 ? (
                <div className={`text-center py-20 rounded-2xl p-6 border ${
                  isDarkMode ? 'bg-gray-900/20 border-gray-850' : 'bg-white border-slate-200 shadow-sm'
                }`} role="status">
                  <Ticket className="w-12 h-12 text-zinc-650 mx-auto mb-4 stroke-1 animate-pulse" aria-hidden="true" />
                  <h4 className={`font-bold font-display ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
                    {activeCategory === 'TicketSwap' ? 'No Available Resales Found' : 'No Companion Requests Found'}
                  </h4>
                  <p className={`text-xs mt-1 leading-normal max-w-sm mx-auto ${isDarkMode ? 'text-gray-500' : 'text-slate-500'}`}>
                    {activeCategory === 'TicketSwap' 
                      ? 'No tickets match your parameters. Switch to user Maya or Priya and click Post Resale to list.'
                      : 'No film buddy requests match your parameters. Click Post Resale and select DayMates to create one!'}
                  </p>
                </div>
              ) : (
                <div className="browse-gallery-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6" aria-labelledby="browse-gallery-title">
                  {filteredListings.map((l) => {
                    const isOwnListing = l.sellerId === activePersona.id;
                    return activeCategory === 'TicketSwap' ? (
                      <TicketSwapListingCard
                        key={l.id}
                        listing={l}
                        isDarkMode={isDarkMode}
                        isOwnListing={isOwnListing}
                        onSelect={handleSelectListingToBuy}
                      />
                    ) : (
                      <DayMatesListingCard
                        key={l.id}
            ...
            <div>
              {viewingChatTxId ? (
                ...
                <div className="max-w-4xl mx-auto px-4 py-8">
                  <h2 className={`text-3xl font-extrabold font-display mb-2 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                    My Active Ticket Swaps
                  </h2>
                  <p className={`text-xs mb-8 ${isDarkMode ? 'text-gray-400' : 'text-slate-600'}`}>
                    Check match statuses, coordinate physically, and release securely held escrow cash.
                  </p>

                  {userTransactions.length === 0 ? (
                    <div className={`text-center py-20 rounded-2xl p-6 border ${
                      isDarkMode ? 'bg-gray-900/20 border-gray-850' : 'bg-white border-slate-200'
                    }`}>
                      <MessageSquare className="w-12 h-12 text-zinc-600 mx-auto mb-4 animate-bounce" />
                      <h4 className={`font-display font-bold ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>No Active Swap Match Initiated</h4>
                      <p className={`text-xs mt-1 max-w-sm mx-auto leading-normal ${isDarkMode ? 'text-gray-500' : 'text-slate-500'}`}>
                        You are not currently processing any buying connections or selling transactions. Go to <b>Browse</b> to buy a ticket!
                      </p>
                    </div>
                  ) : (
                    <div className={`border rounded-2xl overflow-hidden divide-y ${
                      isDarkMode ? 'divide-gray-850 border-gray-850 bg-gray-900/40' : 'divide-slate-100 border-slate-200 bg-white shadow-sm'
                    }`}>
                      {userTransactions.slice().reverse().map((tx) => {
                        const associatedListing = listings.find(l => l.id === tx.listingId);
                        const isBuyerMe = tx.buyerId === activePersona.id;
                        const partnerName = isBuyerMe ? tx.sellerName : tx.buyerName;
                        ...
                        return (
                          <div 
                            ...
                            className={`p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 cursor-pointer transition ${
                              isDarkMode ? 'hover:bg-gray-950/50' : 'hover:bg-slate-50'
                            }`}
                          >
                            <div className="flex items-center gap-3.5">
                              <div className={`w-10 h-10 rounded-full border flex items-center justify-center font-bold shrink-0 text-pink-500 ${
                                isDarkMode ? 'bg-indigo-500/10 border-indigo-500/20' : 'bg-pink-50 border-pink-100'
                              }`}>
                              ...
                              <div>
                                <div className="flex items-center gap-2">
                                  <h4 className={`text-sm font-bold capitalize ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
                                    {associatedListing ? associatedListing.movieName : "Ticket Swap"}
                                  </h4>
                                ...
                                <p className={`text-xs truncate mt-0.5 ${isDarkMode ? 'text-gray-400' : 'text-slate-600'}`}>
                                  Matching Swap partner: <b>{partnerName}</b> ({isBuyerMe ? "You are Buyer" : "You are Seller"})
                                </p>
                                
                                <p className={`text-[10px] font-mono mt-1 flex items-center gap-1.5 uppercase ${
                            ...
                            <div className="flex items-center gap-3 self-end sm:self-auto uppercase">
                              <span className={`font-mono text-xs font-bold mr-1 ${isDarkMode ? 'text-gray-400' : 'text-slate-700'}`}>
                                ₹{associatedListing ? associatedListing.sellingPrice : "0"}
                              </span>
      ...
      <footer className="bg-gray-950 border-t border-gray-850 py-6 text-center text-zinc-500 text-xs">
        <div className="max-w-6xl mx-auto px-4 space-y-2">
          <p>© 2026 TicketSwap Inc. All rights reserved. Platform secure escrow commission operates at default ₹10 per ticket trade transaction.</p>
          <div className="flex justify-center items-center gap-3.5 text-[11px] text-zinc-650 font-mono">
            <span>Development App URL: https://ais-dev-lwuzbioaepmjeoyupuvtj4-692488307747.asia-east1.run.app</span>
            <span>•</span>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl max-w-lg w-full p-6 md:p-8 space-y-6 relative">
            ...
            {checkoutStep === 'DETAILS' && (
              <div className="space-y-6">
                ...
                {/* Title block */}
                <div>
                  {selectedListing.category === 'DayMates' ? (
                    <span className="px-2 py-0.5 bg-indigo-500/10 border border-indigo-500/20 text-[10px] text-indigo-400 font-bold rounded font-mono uppercase">
                      Movie Buddy Companion Search
                    </span>
                  ) : (
                    <span className="px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 text-[10px] text-emerald-400 font-bold rounded font-mono uppercase">
                      You Save ₹{selectedListing.originalPrice - selectedListing.sellingPrice}!
                    </span>
                  )}
                  <h3 className="text-2xl font-bold font-display text-white mt-2 leading-snug">
                    {selectedListing.category === 'DayMates' ? `Movie Companion: ${selectedListing.movieName}` : `Resale: ${selectedListing.movieName}`}
                  </h3>
                ...
                <div className="grid grid-cols-2 gap-4 text-xs font-mono bg-zinc-950/40 p-4 border border-zinc-850 rounded-xl">
                  <div>
                    <span className="text-gray-500 block">Show Time starts:</span>
                    <span className="text-white mt-1 block font-bold">
                      {new Date(selectedListing.showTime).toLocaleString(undefined, { dateStyle: 'short', timeStyle: 'short' })}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-500 block">Seat coordinates:</span>
                  ...
                  <div className="border-t border-zinc-850 pt-2">
                    <span className="text-gray-500 block">
                      {selectedListing.category === 'DayMates' ? 'Total Ticket Cost:' : 'Original Price:'}
                    </span>
                    <span className="text-gray-500 line-through">₹{selectedListing.originalPrice}</span>
                  </div>
                  <div className="border-t border-zinc-850 pt-2">
                    <span className="text-pink-400 block font-bold">
                      {selectedListing.category === 'DayMates' ? 'Companion split cost:' : 'Swap resale price:'}
                    </span>
                    <span className="text-pink-400 text-base font-extrabold font-display">
                      {selectedListing.category === 'DayMates' && selectedListing.sellingPrice === 0 ? 'FREE / SPONSORED' : `₹${selectedListing.sellingPrice}`}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs uppercase font-mono tracking-wider font-bold text-gray-400">
                    {selectedListing.category === 'DayMates' ? 'Select Companion Matching Mode:' : 'Select Your Resale Protection Mode:'}
                  </div>
                    ...
                    <div className="p-4 bg-indigo-950/15 border border-indigo-500/25 rounded-xl space-y-2 flex flex-col justify-between">
                      <div>
                        <h4 className="text-sm font-bold text-indigo-400 font-display flex items-center gap-1">
                          {selectedListing.category === 'DayMates' ? 'Chat Connection' : 'Connect Mode Match'}
                        </h4>
                        <p className="text-gray-400 text-[10.5px] leading-relaxed mt-1">
                          {selectedListing.category === 'DayMates' 
                            ? 'Pay ₹5 platform fee. Connect & chat directly. Meet up at the theatre gate and watch together.'
                            : 'Pay ₹5 platform fee. Connect & chat directly. Swap physically & pay cash/UPI on your own.'}
                        </p>
                      </div>
                      <button
                      ...
                      >
                        {selectedListing.category === 'DayMates' ? 'Connect for ₹5 fee' : 'Swap for ₹5 fee'}
                      </button>
                    </div>
                    ...
                    <div className="p-4 bg-pink-950/15 border border-pink-500/30 rounded-xl border-l-4 border-l-pink-500 space-y-2 flex flex-col justify-between">
                      <div>
                        <h4 className="text-sm font-bold text-pink-400 font-display">
                          {selectedListing.category === 'DayMates' ? 'Safe Split Escrow' : 'Safe Escrow Mode'}
                        </h4>
                        <p className="text-gray-400 text-[10.5px] leading-relaxed mt-1">
                          {selectedListing.category === 'DayMates'
                            ? `Secure split share of ₹${selectedListing.sellingPrice} + ₹10 plat commission held safely on escrow until you rendezvous.`
                            : `Secure funds of ₹${selectedListing.sellingPrice} + ₹10 plat commission held safely on escrow until physical inspection.`}
                        </p>
                      </div>
                      <button
                      ...
                      >
                        {selectedListing.category === 'DayMates'
                          ? `Match Securely for ₹${selectedListing.sellingPrice + 10}`
                ...
                <div className="text-[10px] text-gray-500 text-center leading-normal">
                  Money is deducted from your Swap Wallet. Refill balance in the Wallet tab if funds are low.
                </div>
            ...
            {checkoutStep === 'PAYING' && (
              <div className="py-12 flex flex-col items-center justify-center text-center space-y-6">
                ...
                <div className="space-y-2">
                  <h4 className="text-lg font-bold font-display text-white">Razorpay Secure Integration</h4>
                  <p className="text-gray-405 text-xs">Simulating bank settlement deduction, routing ₹{checkoutMode === 'CONNECT' ? 5 : (selectedListing.sellingPrice + 10)} via Sandbox UPI gateway...</p>
                </div>
              </div>
            )}
              ...
              <div className="py-8 text-center space-y-6">
                ...
                <div className="space-y-2">
                  <h4 className="text-xl font-bold font-display text-white">
                    {selectedListing.category === 'DayMates' ? 'Companion Match Registered!' : 'Resale Match Registered!'}
                  </h4>
                  <p className="text-gray-400 text-xs leading-relaxed max-w-sm mx-auto">
                    {selectedListing.category === 'DayMates' 
                      ? `Simulated buddy match processed successfully via Razorpay API! Match request sent directly to ${selectedListing.sellerName}. Redirecting you to chat...`
                      : `Simulated payment processed successfully via Razorpay API! Match request sent directly to ${selectedListing.sellerName}. Redirecting you to chat...`}
                  </p>
                </div>
                ...
                <div className="flex gap-2 justify-center pt-2">
                  <button
                    onClick={() => {
                      const relevantTx = transactions.find(t => t.listingId === selectedListing.id);
                      setSelectedListing(null);
                      if (relevantTx) {
