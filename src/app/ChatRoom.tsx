...
import { Send, Sparkles, MessageSquare, ShieldCheck, Ticket, User, ClipboardList, ShieldAlert, CheckCircle, ShieldQuestion, HelpCircle, XCircle } from 'lucide-react';
import { Transaction, Listing, ChatMessage } from '../shared/types';
...
export default function ChatRoom({ transactionId, activeUserId, activeUserName, onStatusUpdate, isDarkMode = true }: ChatRoomProps) {
  ...
  const fetchChatDetails = async () => {
    try {
      ...
      const tx: Transaction = stats.transactions.find((t: any) => t.id === transactionId);
      if (tx) {
        setTransaction(tx);
  ...
  const handleSellerResponse = async (action: 'ACCEPT' | 'REJECT') => {
    try {
      ...
      if (res.ok) {
        setSuccessMsg(`You have successfully ${action === 'ACCEPT' ? 'accepted' : 'rejected'} the request.`);
        fetchChatDetails();
        onStatusUpdate();
  ...
  const handleSimulateTimeout = async () => {
    try {
      const res = await fetch(`/api/transactions/${transactionId}/timeout`, { method: "POST" });
      if (res.ok) {
        setSuccessMsg("10 minute timeout simulated. Platform fee refunded back to buyer wallet!");
        fetchChatDetails();
        onStatusUpdate();
  ...
  const handleCancelTransaction = async () => {
    const reason = prompt("Enter reason for cancellation (Buyer inspection mismatch / meet failure):");
    if (reason === null) return;
    try {
      const res = await fetch(`/api/transactions/${transactionId}/cancel`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ reason: reason || "Negotiations failed during inspection", userId: activeUserId })
      });
      if (res.ok) {
  ...
  const handleVerifyOTP = async (verifyRole: 'BUYER_INPUT_SELLER_OTP' | 'SELLER_INPUT_BUYER_OTP') => {
    setErrorMsg("");
    if (!otpInput) {
    ...
    try {
      const res = await fetch(`/api/transactions/${transactionId}/otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ otp: otpInput, role: verifyRole, userId: activeUserId })
      });
      if (res.ok) {
        setOtpInput("");
  ...
  const handleRaiseDispute = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/transactions/${transactionId}/dispute`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ reason: disputeReason, userId: activeUserId, userName: activeUserName })
      });
      if (res.ok) {
        setShowDisputeModal(false);
        setSuccessMsg("Dispute officially raised. Escrow funds are now locked under REVIEW state!");
        fetchChatDetails();
        onStatusUpdate();
        setTimeout(() => setSuccessMsg(""), 3500);
      }
  ...
  const handleSimulateSettle = async () => {
    try {
      const res = await fetch(`/api/transactions/${transactionId}/settle`, { method: "POST" });
      if (res.ok) {
        setSuccessMsg("60-minute window elapsed. Funds successfully settled to Seller wallet!");
        fetchChatDetails();
        onStatusUpdate();
        setTimeout(() => setSuccessMsg(""), 3500);
      }
  ...
  const triggerAiResponse = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setErrorMsg("Gemini API is unavailable right now. Executing offline chatbot fallback instead.");
        // retry local
        fetchChatDetails();
      }
    } catch (err) {
  ...
  const showSellerActionForm = isSeller && transaction.status === 'MATCH_REQUESTED';

  return (
    <div className={`chat-container max-w-6xl mx-auto px-2 sm:px-4 py-4 sm:py-6 h-auto lg:h-[85vh] ${
        ...
        >
          <div>
            ...
            <div className={`pb-4 border-b mb-4 ${
              ...
              <h3 className={`text-base font-bold font-display mt-1.5 leading-snug ${
            ...
            <div className="space-y-4" aria-live="polite" aria-atomic="true">
              ...
              {transaction.status === 'MATCH_REQUESTED' && (
                <div className={`border rounded-xl p-4 ${
                  ...
                  <p className={`text-xs leading-relaxed mt-1.5 ${
                    ...
                    The checkout is held. If you are Maya/Priya, you can Accept or Reject this deal.
                  </p>
                  
                  {transaction.mode === 'CONNECT' && (
                    <div className={`mt-3 pt-3 border-t ${
                      isDarkMode ? 'border-yellow-900/30' : 'border-amber-200/50'
                    }`}>
                      <p className={`text-[10px] font-mono ${
                        isDarkMode ? 'text-yellow-500' : 'text-amber-600'
                      }`}>CONNECT COUNTER TIMEOUT (10m)</p>
                      <button
              ...
              {showSellerActionForm && (
                ...
                }`} role="group" aria-label="Seller validation action controls">
                  <p className={`font-semibold font-display text-xs ${
              ...
              {transaction.mode === 'SAFE' && (transaction.status === 'MEETING_SCHEDULED' || transaction.status === 'OTP_PENDING') && (
                <div className={`border rounded-xl p-4 space-y-4 ${
                  isDarkMode 
                    ? 'bg-indigo-950/20 border-indigo-500/20 text-indigo-300' 
                    : 'bg-indigo-50/50 border-indigo-150 text-indigo-900'
                }`} role="region" aria-label="Meeting physical check rules">
                  <div className="flex items-center gap-1.5">
                    <Ticket className={`w-4 h-4 shrink-0 ${isDarkMode ? 'text-pink-400' : 'text-pink-600'}`} aria-hidden="true" />
                    <p className={`font-bold text-xs font-display ${isDarkMode ? 'text-indigo-300' : 'text-indigo-700'}`}>
                      Step 2: Meeting Venue Inspection
                    </p>
                  </div>
                  <p className={`text-[11px] leading-relaxed ${isDarkMode ? 'text-gray-450' : 'text-slate-600'}`}>
                    Meet at the **Theatre Entrance/Gate of {listing?.theatreName}**. 
                    Verify booking confirmation codes, seat numbers, and dates. Apply OTPs below when satisfied.
                  </p>
                  ...
                  <div className={`p-3 rounded-lg border space-y-2 text-xs ${
                    ...
                    <div className={`flex justify-between border-b pb-2 ${
                      ...
                      <span className={isDarkMode ? 'text-gray-400' : 'text-slate-500'}>Your Verification OTP:</span>
                      <span className={`font-mono font-bold text-sm tracking-widest leading-none ${isDarkMode ? 'text-pink-400' : 'text-pink-600'}`} aria-label={`Your verification OTP is ${isBuyer ? transaction.buyerOtp : transaction.sellerOtp}`}>
                        {isBuyer ? transaction.buyerOtp : transaction.sellerOtp}
                      </span>
                    </div>
                    <div className={`text-[10px] ${isDarkMode ? 'text-gray-450' : 'text-slate-400'}`}>
                      {isBuyer ? "Share this with Maya physically once she verifies your payment status." : "Share this with Raghu once he physically inspects your booking QR."}
                    </div>
                  </div>

                  {/* INPUT opposite OTP code */}
                  <div className="space-y-2">
                    <label htmlFor="opposite-party-otp" className={`block text-[10px] font-bold font-mono ${isDarkMode ? 'text-gray-300' : 'text-slate-600'}`}>
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <button
                        onClick={() => handleVerifyOTP(isBuyer ? 'BUYER_INPUT_SELLER_OTP' : 'SELLER_INPUT_BUYER_OTP')}
              ...
              {transaction.status === 'DISPUTE_WINDOW' && (
                ...
                }`} role="region" aria-label="60-minute dispute filing window controls">
                  <div className="flex items-center gap-1.5">
                    <ShieldCheck className="w-4.5 h-4.5" aria-hidden="true" />
                    <span className="font-bold text-xs font-display">60m Dispute Window Active</span>
                  </div>
                  <p className={`text-[11px] leading-relaxed ${isDarkMode ? 'text-gray-450' : 'text-emerald-900/80'}`}>
                    Buyer inspects entry scan status inside theater hall doors. If you discover double booking or invalid barcode scan failures, raise a dispute within 60 mins.
                  </p>

                  <div className={`grid grid-cols-1 gap-2 pt-2 border-t ${
                    isDarkMode ? 'border-emerald-900/30' : 'border-emerald-200/50'
                  }`}>
                    <button
              ...
              {transaction.status === 'REVIEW' && (
                <div className={`border p-4 rounded-xl ${
                  ...
                  <p className={`text-[11px] leading-relaxed mt-2 ${isDarkMode ? 'text-gray-440' : 'text-red-700'}`}>
                    Escrow funds of ₹{transaction.amountPaid} are frozen temporarily. 
                    Admin (log in as Admin persona) can review the claims and handle resolution refunds.
                  </p>
                </div>
              )}
              ...
              {transaction.status === 'COMPLETED' && (
                <div className={`border p-4 rounded-xl text-center space-y-2 ${
                  isDarkMode 
                    ? 'bg-indigo-950/20 border-indigo-500/20 text-indigo-300' 
                    : 'bg-indigo-50 border-indigo-150 text-indigo-900'
                }`} role="status">
                  <CheckCircle className={`w-8 h-8 mx-auto ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`} aria-hidden="true" />
                  <h4 className="font-bold text-xs font-display">Resale Completed Successfully</h4>
                  <p className={`text-[10.5px] leading-relaxed ${isDarkMode ? 'text-gray-450' : 'text-slate-500'}`}>
                    Connect chat/Safe Escrow complete. Earnings paid out, fee resolved. Thanks for using TicketSwap!
                  </p>
                </div>
              )}
          ...
          {/* Quick Tips */}
          <div className={`pt-4 border-t hidden lg:block text-[10.5px] space-y-1 ${
            isDarkMode 
              ? 'border-gray-850/60 text-gray-500' 
              : 'border-slate-100 text-slate-400'
          }`}>
            <p className={`font-semibold ${isDarkMode ? 'text-gray-400' : 'text-slate-500'}`}>Swap Instructions:</p>
        ...
        >
          ...
          <div className={`p-4 border-b flex items-center justify-between gap-4 ${
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-full bg-pink-600 flex items-center justify-center font-bold text-white uppercase" aria-hidden="true">
                {isBuyer ? "M" : "R"}
              </div>
              <div>
                <p className={`text-xs font-bold leading-none ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
                  {isBuyer ? transaction.sellerName : transaction.buyerName}
                </p>
                <p className={`text-[10px] uppercase font-mono mt-1 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>
                  Active Match Portal
                </p>
              </div>
            </div>
          ...
          >
            ...
            {messages.map((msg) => {
              if (msg.isSystem) {
                return (
                  <div key={msg.id} className="w-full flex justify-center py-2" role="note">
                    <div className={`border text-[11px] leading-relaxed p-3 rounded-xl max-w-md text-center shadow-md font-mono ${
                      isDarkMode 
              ...
              return (
                <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`} role="status">
                  <div className={`chat-message-item p-3 rounded-xl max-w-sm text-xs leading-relaxed ${
                    isMe 
                    ...
                    <p className={`font-mono text-[9px] mb-0.5 ${isMe ? 'text-pink-200' : (isDarkMode ? 'text-zinc-400' : 'text-slate-500')}`}>{msg.senderName}</p>
                    <p className="whitespace-pre-line">{msg.text}</p>
                    <span className={`text-[9px] block text-right mt-1.5 font-mono ${isMe ? 'text-pink-200' : (isDarkMode ? 'text-zinc-500' : 'text-slate-400')}`}>
          ...
          <div aria-live="assertive" aria-relevant="all" className="empty:hidden">
            {(errorMsg || successMsg) && (
              <div 
                ...
                role="alert"
              >
                <span className={errorMsg ? "text-rose-400" : "text-emerald-400"}>
                  {errorMsg || successMsg}
                </span>
                <button 
      ...
      {showDisputeModal && (
        ...
        >
          <div className={`border rounded-2xl max-w-md w-full p-6 space-y-4 shadow-3xl text-left ${
            isDarkMode 
              ? 'bg-gray-900 border-gray-800 text-white' 
              : 'bg-white border-slate-200 text-slate-800'
          }`}>
            <div className="flex justify-between items-center">
              <h3 id="dispute-title" className={`text-base font-bold font-display flex items-center gap-1.5 ${
                isDarkMode ? 'text-white' : 'text-slate-900'
              }`}>
            ...
            <p id="dispute-desc" className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-440' : 'text-slate-500'}`}>
              Escrow payment holds will be frozen completely under REVIEW status. Admin will study details to mediate refund arbitrations.
            </p>

            <form onSubmit={handleRaiseDispute} className="space-y-4">
              <div>
                <label htmlFor="dispute-category-select" className={`block text-[10px] font-bold font-mono mb-2 uppercase tracking-wider ${
                ...
                >
                  ...
                  <option value="Duplicate Ticket: The scanner barcode said the ticket has already been used by someone else inside.">Duplicate ticket (already scanned / used)</option>
                  <option value="Invalid QR Code: The booking reference ticket code is fake or does not exist on BookMyShow/Paytm database.">Fake booking confirmation code / invalid QR code</option>
                  <option value="Wrong Seating/Theatre: ticket seats differ from Box Row F1 or movie name is wrong film entirely.">Incorrect Theatre / Seating Box coords</option>
                  <option value="Seller Fraud: The show commenced but the seller showed duplicate codes or spoofed confirmation codes.">Seller fraud or physical misbehaviour at gate</option>
                </select>
              </div>
