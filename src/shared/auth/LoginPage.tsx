...
import { 
  Ticket, Sparkles, Eye, EyeOff, Loader2, 
  ArrowRight, Phone, CheckCircle, RefreshCw,
...
export default function LoginPage({ onLoginSuccess, allPersonas, onRegisterSuccess }: LoginPageProps) {
  const [formState, setFormState] = useState({
    activeFormTab: 'LOGIN_MOBILE' as 'LOGIN_MOBILE' | 'REGISTER' | 'FORGOT_PASSWORD',
    mobileInput: "",
    otpInput: "",
    passwordInput: "",
    nameInput: "",
    emailInput: "",
    registerRole: 'buyer' as 'buyer' | 'seller',
    loginMethod: 'OTP' as 'OTP' | 'GOOGLE',
    loginEmail: "",
    loginPassword: "",
    showPassword: false,
    showSandbox: false,
    sentOtp: null as string | null,
  ...
  const updateState = (updates: Partial<typeof formState>) => {
  ...
  const handleRequestOtp = () => {
    if (!mobileInput || mobileInput.length < 10) {
      updateState({ errorMsg: "Please enter a valid 10-digit mobile number first." });
      return;
    }
    updateState({
      otpSentLoading: true,
    ...
    startTransition(async () => {
      try {
        const data = await request<{ simulationOtp: string }>("/api/auth/request-otp", {
          method: "POST",
          body: { mobileNumber: mobileInput },
        });
        updateState({
          sentOtp: data.simulationOtp,
          successMsg: `Simulated OTP text sent: ${data.simulationOtp} (Use this code to login!).`,
          otpInput: data.simulationOtp
        });
      } catch (err: any) {
        if (err.name !== "AbortError") {
          updateState({ errorMsg: err.message || "Network or gateway connection timeout." });
        }
      } finally {
        updateState({ otpSentLoading: false });
      }
    });
  };
  ...
  const handleMobileLoginSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!mobileInput) {
      updateState({ errorMsg: "Enter your registered Indian mobile number." });
      return;
    }
    if (!otpInput && !passwordInput) {
      updateState({ errorMsg: "Enter either the OTP code or your passcode to authenticate." });
      return;
    }

    updateState({ errorMsg: "", successMsg: "" });

    startTransition(async () => {
      try {
        const data = await request<{ user: Persona }>("/api/auth/login-mobile", {
          method: "POST",
          body: {
        ...
        updateState({ successMsg: `Authenticated successfully! Welcome as ${data.user.name}.` });
        setTimeout(() => {
          onLoginSuccess({
            id: data.user.id,
            name: data.user.name,
        ...
        if (err.name !== "AbortError") {
          updateState({ errorMsg: err.message || "Verification failed. Check your security entries." });
        }
      }
    });
  };
  ...
  const handleEmailLoginSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!loginEmail) {
      updateState({ errorMsg: "Please enter your registered email address." });
      return;
    }
    if (!loginPassword) {
      updateState({ errorMsg: "Please enter your passcode." });
      return;
    }
    ...
    startTransition(async () => {
      try {
        ...
        updateState({ successMsg: `Authenticated successfully! Welcome, ${data.user.name}.` });
        setTimeout(() => {
          onLoginSuccess({
            id: data.user.id,
            name: data.user.name,
        ...
        if (err.name !== "AbortError") {
          updateState({ errorMsg: err.message || "Authentication failed. Check your email and passcode." });
        }
      }
    });
  };
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    if (!nameInput || !mobileInput || !passwordInput) {
      updateState({ errorMsg: "Please complete the required fields: Name, Phone, and Passcode." });
      return;
    }
    ...
    startTransition(async () => {
      try {
        const data = await request<{ user: Persona, alreadyRegistered?: boolean }>("/api/auth/register", {
          method: "POST",
          body: {
            name: nameInput.trim(),
            mobileNumber: mobileInput,
        ...
        const welcomeMsg = data.alreadyRegistered
        ...
        updateState({ successMsg: welcomeMsg });
        ...
        if (err.name !== "AbortError") {
          updateState({ errorMsg: err.message || "Check your formatting or use another phone number." });
        }
      }
    });
  };
  ...
  const handleForgotPasswordSubmit = (e: React.FormEvent) => {
    ...
    if (!mobileInput || !passwordInput) {
      updateState({ errorMsg: "Please fill both mobile number and new passcode." });
      return;
    }
    ...
    startTransition(async () => {
      try {
        const data = await request<{ message: string }>("/api/auth/forgot-password", {
          method: "POST",
          body: {
            mobileNumber: mobileInput,
        ...
        updateState({ successMsg: data.message || "Passcode updated successfully!" });
        setTimeout(() => {
          updateState({
        ...
        if (err.name !== "AbortError") {
          updateState({ errorMsg: err.message || "Reset failed. Verify that your mobile is registered." });
        }
      }
    });
  };
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col justify-center items-center px-4 py-8 relative overflow-hidden font-sans">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[420px] h-[420px] bg-gradient-to-tr from-pink-600/15 via-rose-600/10 to-indigo-600/15 rounded-full blur-3xl -z-10 animate-pulse duration-[8000ms]" aria-hidden="true" />
      ...
      <main className="max-w-md w-full glass-card border border-gray-850 p-7 rounded-2xl relative overflow-hidden shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          <div className="inline-flex p-3 bg-gradient-to-tr from-pink-600 to-indigo-600 rounded-2xl text-white shadow shadow-pink-900/30" aria-hidden="true">
        ...
        <div aria-live="assertive" className="empty:hidden space-y-2">
          ...
          {successMsg && (
            <div className="p-3 bg-emerald-500/10 border border-emerald-500/25 text-emerald-400 text-xs rounded-xl flex items-center gap-2.5" role="status">
              <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0" aria-hidden="true" />
              <span className="leading-snug font-mono whitespace-pre-line">{successMsg}</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <GoogleSsoButton
            ...
            setErrorMsg={(msg) => updateState({ errorMsg: msg })}
            setSuccessMsg={(msg) => updateState({ successMsg: msg })}
          />
          ...
          <details className="group border border-gray-850 rounded-xl bg-gray-900/30 overflow-hidden">
            ...
            <div className="p-4 border-t border-gray-850 space-y-3 bg-gray-950/40 animate-fadeIn">
              <p className="text-[10px] text-zinc-400">
                Select a pre-loaded Indian cinema profile to log in in exactly <b>one click</b>:
              </p>
              <div className="grid grid-cols-2 gap-2">
                {allPersonas.map((p) => (
                  ...
                  >
                    <img src={p.avatar} className="w-7 h-7 rounded-full object-cover shrink-0 border border-gray-800" alt="" />
                    <div className="truncate text-left">
                      <p className="font-bold text-white text-xs group-hover:text-pink-400 capitalize truncate">
                        {p.name.split(' ')[0]}
                      </p>
                      <p className="text-[9px] text-zinc-500 font-mono capitalize leading-none mt-0.5">{p.role}</p>
                    </div>
                  </button>
                ))}
              </div>
        ...
        <div className="border border-gray-850 rounded-xl bg-gray-900/30 overflow-hidden">
          <button
            type="button"
            onClick={() => updateState({ showSandbox: !showSandbox })}
          ...
          {showSandbox && (
            <div className="p-4 border-t border-gray-850 space-y-4 bg-gray-950/40">
              ...
              <div className="grid grid-cols-3 gap-1 bg-gray-950 p-1 rounded-lg border border-gray-850 mb-2">
                <button
                  type="button"
                  onClick={() => updateState({ activeFormTab: 'LOGIN_MOBILE' })}
                ...
                <button
                  type="button"
                  onClick={() => updateState({ activeFormTab: 'REGISTER' })}
                ...
                <button
                  type="button"
                  onClick={() => updateState({ activeFormTab: 'FORGOT_PASSWORD' })}
              ...
              {activeFormTab === 'LOGIN_MOBILE' && (
                <form onSubmit={handleMobileLoginSubmit} className="space-y-3">
                  <div>
                    <label htmlFor="login-mobile" className="block text-gray-450 font-mono text-[9px] uppercase tracking-wider mb-1 font-bold">
                      Indian Mobile Number
                    </label>
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-550 text-xs" aria-hidden="true">
                        ...
                        <input
                          ...
                          value={mobileInput}
                          onChange={(e) => updateState({ mobileInput: e.target.value })}
                        />
                      </div>
                      <button
                        type="button"
                      ...
                      >
                        {otpSentLoading ? <RefreshCw className="w-3 h-3 animate-spin" /> : "Send OTP"}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label htmlFor="login-otp" className="block text-gray-450 font-mono text-[9px] uppercase tracking-wider mb-1 font-bold">
                        6-Digit OTP Code
                      </label>
                      <input
                        id="login-otp"
                    ...
                    <div>
                      <label htmlFor="login-passcode" className="block text-gray-450 font-mono text-[9px] uppercase tracking-wider mb-1 font-bold">
                        Passcode (Optional)
                      </label>
                      <input
                        id="login-passcode"
                        ...
                        className="w-full px-3 py-1.5 bg-gray-950 border border-gray-850 rounded-lg text-white text-xs focus:border-pink-500 focus:outline-none"
                        value={passwordInput}
                        onChange={(e) => updateState({ passwordInput: e.target.value })}
                      />
                    </div>
                  </div>
              ...
              {activeFormTab === 'REGISTER' && (
                <form onSubmit={handleRegisterSubmit} className="space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label htmlFor="reg-name" className="block text-gray-450 font-mono text-[9px] uppercase tracking-wider mb-1 font-bold">
                        Full Name
                      </label>
                      <input
                        id="reg-name"
                        ...
                        className="w-full px-3 py-1.5 bg-gray-950 border border-gray-850 rounded-lg text-white text-xs focus:border-pink-500 focus:outline-none"
                        value={nameInput}
                        onChange={(e) => updateState({ nameInput: e.target.value })}
                      />
                    </div>

                    <div>
                      <label htmlFor="reg-mobile" className="block text-gray-450 font-mono text-[9px] uppercase tracking-wider mb-1 font-bold">
                        Mobile Number
                      </label>
                      <input
                        id="reg-mobile"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label htmlFor="reg-pass" className="block text-gray-450 font-mono text-[9px] uppercase tracking-wider mb-1 font-bold">
                        Password (6+ chars)
                      </label>
                      <input
                        id="reg-pass"
                    ...
                    <div>
                      <label htmlFor="reg-email" className="block text-gray-450 font-mono text-[9px] uppercase tracking-wider mb-1 font-bold">
                        Email (Optional)
                      </label>
                      <input
                        id="reg-email"
                        ...
                        className="w-full px-3 py-1.5 bg-gray-950 border border-gray-850 rounded-lg text-white text-xs focus:border-pink-500 focus:outline-none"
                        value={emailInput}
                        onChange={(e) => updateState({ emailInput: e.target.value })}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={() => updateState({ registerRole: 'buyer' })}
                        className={`flex-1 py-1.5 px-2 border text-xs font-semibold rounded-lg text-center transition cursor-pointer ${
                          registerRole === 'buyer' ? 'bg-pink-600/20 border-pink-550 text-pink-400' : 'bg-gray-950 border-gray-850 text-gray-400'
                        }`}
                      >
                        I want to Buy
                      </button>
                      <button
                        type="button"
                        onClick={() => updateState({ registerRole: 'seller' })}
                        className={`flex-1 py-1.5 px-2 border text-xs font-semibold rounded-lg text-center transition cursor-pointer ${
                          registerRole === 'seller' ? 'bg-pink-600/20 border-pink-550 text-pink-400' : 'bg-gray-950 border-gray-850 text-gray-400'
                        }`}
                      >
                        I want to Sell
                      </button>
              ...
              {activeFormTab === 'FORGOT_PASSWORD' && (
                <form onSubmit={handleForgotPasswordSubmit} className="space-y-3">
                  <div>
                    <label htmlFor="forgot-mobile" className="block text-gray-450 font-mono text-[9px] uppercase tracking-wider mb-1 font-bold">
                  ...
                  <div>
                    <label htmlFor="forgot-pass" className="block text-gray-450 font-mono text-[9px] uppercase tracking-wider mb-1 font-bold">
                      Choose New Passcode
                    </label>
                    <input
                      id="forgot-pass"
