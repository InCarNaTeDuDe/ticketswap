...
export default function ChatRoom({ transactionId, activeUserId, activeUserName, onStatusUpdate }: ChatRoomProps) {
  ...
  return (
    <div className="chat-container max-w-6xl mx-auto px-2 sm:px-4 py-4 sm:py-6 text-white h-auto lg:h-[80vh]">
      
      <div className="chat-layout-grid grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 h-full">
        ...
        <section 
          ...
          className="chat-control-panel lg:col-span-4 bg-gray-900/60 border border-gray-850 rounded-2xl p-4 sm:p-5 flex flex-col justify-between overflow-y-auto"
        >
          <div>
            {/* Status Header info */}
            <div className="pb-4 border-b border-gray-850 mb-4" id="chat-swap-header">
              <span className="px-2 py-0.5 rounded font-mono font-bold text-[9px] bg-pink-500/10 text-pink-400 border border-pink-500/20 uppercase" aria-label={`Security mode is ${transaction.mode}`}>
            ...
            <div className="space-y-4" aria-live="polite" aria-atomic="true">
              ...
              {transaction.status === 'MATCH_REQUESTED' && (
                <div className="bg-yellow-950/20 border border-yellow-550/20 rounded-xl p-4" role="status">
                  ...
                  {transaction.mode === 'CONNECT' && (
                    <div className="mt-3 pt-3 border-t border-yellow-900/30">
                      <p className="text-[10px] text-yellow-500 font-mono">CONNECT COUNTER TIMEOUT (10m)</p>
                      <button
              ...
              {showSellerActionForm && (
                <div className="p-4 bg-gray-950 border border-pink-500/20 rounded-xl space-y-3" role="group" aria-label="Seller validation action controls">
              ...
              {transaction.mode === 'SAFE' && (transaction.status === 'MEETING_SCHEDULED' || transaction.status === 'OTP_PENDING') && (
                <div className="bg-indigo-950/20 border border-indigo-500/20 rounded-xl p-4 space-y-4" role="region" aria-label="Meeting physical check rules">
                  <div className="flex items-center gap-1.5">
                    <Ticket className="w-4 h-4 text-pink-400 shrink-0" aria-hidden="true" />
                  ...
                  <div className="bg-gray-950/80 p-3 rounded-lg border border-gray-850 space-y-2 text-xs">
                    <div className="flex justify-between border-b border-gray-850 pb-2">
                      <span className="text-gray-450">Your Verification OTP:</span>
                      <span className="font-mono font-bold text-pink-400 text-sm tracking-widest leading-none" aria-label={`Your verification OTP is ${isBuyer ? transaction.buyerOtp : transaction.sellerOtp}`}>
                  ...
                  <div className="space-y-2">
                    <label htmlFor="opposite-party-otp" className="block text-gray-300 text-[10px] font-bold font-mono">
              ...
              {transaction.status === 'DISPUTE_WINDOW' && (
                <div className="bg-emerald-950/20 border border-emerald-500/20 rounded-xl p-4 space-y-3.5" role="region" aria-label="60-minute dispute filing window controls">
              ...
              {transaction.status === 'REVIEW' && (
                <div className="bg-red-950/30 border border-red-500/30 p-4 rounded-xl" role="alert">
              ...
              {transaction.status === 'COMPLETED' && (
                <div className="bg-indigo-950/20 border border-indigo-500/20 p-4 rounded-xl text-center space-y-2" role="status">
                  <CheckCircle className="w-8 h-8 text-indigo-400 mx-auto" aria-hidden="true" />
                  <h4 className="text-indigo-300 font-bold text-xs font-display">Resale Completed Successfully</h4>
        ...
        <section 
          ...
          className="chat-message-panel lg:col-span-8 bg-gray-900/40 border border-gray-850 rounded-2xl flex flex-col justify-between overflow-hidden relative"
        >
              ...
              return (
                <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`} role="status">
                  <div className={`chat-message-item p-3 rounded-xl max-w-sm text-xs leading-relaxed ${
                    isMe ? 'bg-pink-600 text-white rounded-tr-none' : 'bg-gray-850 text-gray-200 rounded-tl-none'
                  }`}>
                    <p className="font-mono text-[9px] text-zinc-400 mb-0.5">{msg.senderName}</p>
        ...
        >
          <div className="bg-gray-900 border border-gray-800 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-3xl text-left">
            <div className="flex justify-between items-center">
              <h3 id="dispute-title" className="text-base font-bold font-display text-white flex items-center gap-1.5">
            ...
            <p id="dispute-desc" className="text-gray-440 text-xs leading-relaxed">
            ...
            <form onSubmit={handleRaiseDispute} className="space-y-4">
              <div>
                <label htmlFor="dispute-category-select" className="block text-gray-300 text-[10px] font-bold font-mono mb-2 uppercase tracking-wider">
